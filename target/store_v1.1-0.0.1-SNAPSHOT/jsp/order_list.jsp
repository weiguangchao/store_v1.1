						...
						<c:forEach items="${requestScope.page.list}" var="order">
							<tbody>
								<tr class="success">
									<th colspan="5">
										订单编号:&nbsp;${order.oid}&nbsp;&nbsp;
										总价格:&nbsp;<font color="blue">${order.total}</font>&nbsp;&nbsp;
										<c:if test="${order.state==1}">
											<a style="color: red" href="${pageContext.request.contextPath}/order?method=getOrder&oid=${order.oid}">结账</a>
										</c:if>
										<c:if test="${order.state==2}">
											<a style="color: green" href="javascript:void(0)">已付款</a>
										</c:if>
									</th>
								</tr>
								<tr class="warning">
